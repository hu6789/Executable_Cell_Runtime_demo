...
def compute_passive_formula(
    ...
    if f_type == "proportional_leakage":
        ...
        return (
            node_values.get(src, 0.0)
            * node_values.get(inst, 1.0)
            * rate
        )
    ...
    if f_type == "resource_consumption":
        activity_node = formula.get("activity", "protein")
        load_node = formula.get("load", "RNA")
        rate = formula.get("consumption_rate", 0.1)
        act = node_values.get(activity_node, 0.0)
        load = node_values.get(load_node, 0.0)
